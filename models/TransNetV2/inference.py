...
class TransNetV2Engine:
    ...
    def __init__(self, model_dir=None):
        ...
        try:
            ...
            self._model.load_state_dict(torch.load(os.path.join(model_dir, "transnetv2-pytorch-weights.pth"), map_location="cpu"))
            self._model.eval()
            torch.no_grad()
    ...
    def __call__(self, batch: Dict[str,np.ndarray], **kwds):
            print(batch.keys())
            print(batch["data"].shape)
            assert len(batch["data"].shape) == 4 and batch["data"].shape[1:] == self._input_size, \
                "[TransNetV2] Input shape must be [frames, height, width, 3]."
            single_frame_pred, all_frames_pred = self.predict_frames(batch["data"])
            return {"scenes":  self.predictions_to_scenes(single_frame_pred)}
    ...
    def predict_raw(self, frames: np.ndarray):
        ...
        all_frames_pred = torch.sigmoid(dict_["many_hot"])
    ...
    def predict_frames(self, frames: np.ndarray):
        ...
        for inp in input_iterator():
            ...
            predictions.append((single_frame_pred.detach().numpy()[0, 25:75, 0],
                                all_frames_pred.detach().numpy()[0, 25:75, 0]))
...
def main():
    ...
    model = TransNetV2Engine(args.weights)
